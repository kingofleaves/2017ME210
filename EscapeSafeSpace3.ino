...
#define IRRearThreshold 570                                       // CHANGE THIS THRESHOLD AS NECESSARY 
...
void stage1(void){
  ...
  while(IRRearReading < IRRearThreshold){                             
      handleMotors(STATE_PIVOT_R, TURN_INTERVAL, 40);     // originally: 3, 20
      IRRearReading = analogRead(IR_REAR);
      Serial.println(IRRearReading);
  }
  ...
  while(!sensorRightDark() || !sensorLeftDark()) {
    ...
    if (sensorRightDark()) {
      ...
      handleMotors(STATE_FORWARD, 256, FORWARD_PULSE);  // stops the motor 



  // experimental - move forward for 1sec 
   TMRArd_InitTimer(6, 300);

   while (!TMRArd_IsTimerExpired(6)) {
        handleMotors(STATE_FORWARD, 1, FORWARD_PULSE);
  ...
  // Then, move forward until it finds the line 
  while(!sensorRightDark() && !sensorLeftDark()) {
    handleMotors(STATE_FORWARD, FORWARD_INTERVAL, FORWARD_PULSE);
    } 
  ...
  while(!sensorRightDark() || !sensorLeftDark()) {
    ...
    if (sensorRightDark()) {
      ...
      handleMotors(STATE_FORWARD, FORWARD_INTERVAL, 25);
    }
    } 
    
    // Corrector - moves a teeny bit more in its last direction 
  
if (lastright==1){
     TMRArd_InitTimer(7, 300);
          Serial.println("corecting right");
   while (!TMRArd_IsTimerExpired(7)) {
      handleMotors(STATE_PIVOT_R, TURN_INTERVAL, TURN_PULSE);
  }
}
if (lastleft==1){
     TMRArd_InitTimer(7, 300);
               Serial.println("corecting left");

   while (!TMRArd_IsTimerExpired(7)) {
      handleMotors(STATE_PIVOT_L, TURN_INTERVAL, TURN_PULSE);
  }
}
  // When it reaches here, it's aligned with the line. 
      Serial.println("stopped");
      // enter new state here 
  ...
  if(TMRArd_IsTimerExpired(TIMER_PULSE)) {
    ...
    if (waitCount < pulseInterval) {
      ...
      if (currState == STATE_LEFT) {
        ...
        analogWrite(PIN_MOTOR_LEFT, MOTOR_PULSE_SPEED + 10);
        //Serial.println("LEFT ++");
      }
      if (currState == STATE_RIGHT) {
        ...
        analogWrite(PIN_MOTOR_RIGHT, MOTOR_PULSE_SPEED + 10);
